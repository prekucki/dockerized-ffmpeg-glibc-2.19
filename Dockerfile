...
COPY freetype-2.5.5.tar.gz /tmp/
RUN  \
        DIR=/tmp/freetype && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
	cp /tmp/freetype-2.5.5.tar.gz . && \
        echo ${FREETYPE_SHA256SUM} https://download.savannah.gnu.org/releases/freetype/freetype-${FREETYPE_VERSION}.tar.gz && \
	sha256sum freetype-${FREETYPE_VERSION}.tar.gz && \
...
RUN \
	...
	mkdir -p ${DIR} && \
	cd ${DIR} && \
	curl -sLO https://github.com/Kitware/CMake/releases/download/v3.14.1/cmake-3.14.1.tar.gz && \
	tar -zx --strip-components=1 -f cmake-3.14.1.tar.gz && \
	./bootstrap && \
	make -j4 && \
	make install && \
	rm -fr ${DIR}
